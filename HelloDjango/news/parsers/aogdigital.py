...
URL = 'https://aogdigital.com/news/latest'
HOST = 'https://aogdigital.com'


def get_content(html):
    soup = BeautifulSoup(html, 'html.parser')
    try:
        items = soup.find('div', class_='article').find_all('a', class_='snippet')
    except:
        items = None
        ...
        for item in items:
            try:
                ...
                if has_post.count():
                    ...
                    continue
                try:
                    image_url = item_soup.find('div', class_='article').find('img').get('src')
                except:
                    image_url = None
                body = item_soup.find('div', property='articleBody')
            except:
