...
URL = 'https://www.kazenergy.com/ru/press-center/news/'
HOST = 'https://www.kazenergy.com'


def get_content(html):
    soup = BeautifulSoup(html, 'html.parser')
    try:
        items = soup.find_all('div', class_='section-item')
    except:
        items = None
    ...
    if items is not None:
        for item in items:
            try:
                ...
                try:
                    image_url = HOST + item_soup.find('div', class_='detail-text').find('img').get('src')
                except:
                    image_url = None
                body = item_soup.find('div', class_='detail-text')
